...
public:
    ...
    vector<Arista<T1>*> Dijkstra(int nodoBusqueda);
    void Generar_Aristas_Invertidas();
    Grafo<T1>* Invertir_Aristas();
    bool Es_Fuertemente_Conexo();
    vector<Arista<T1>*> Busqueda_A(int id_inicio, int id_final);
    int Get_aristas();
...
template<typename T1>
vector<Arista<T1>*> Grafo<T1>::Dijkstra(int search_id) {
	...
	for (int i = 0; i < nodos.size(); i++) {
		...
		MatrizSolucionesDos.push_back(0);
	}

	int posicion = 0, indice;
	for (int i = 0; i < nodos.size(); i++) {
	...
	for (int i = 0; i < nodos.size(); i++) {
		cout << MatrizSoluciones[i]<< "    " << nodos[MatrizSolucionesDos[i]]->data->id << "       " << nodos[i]->data->id << endl;
	}
	vector<Arista<T1>*> aristas;
	for (int i = 0; i < this->nodos.size(); i++) {
    if(MatrizSoluciones[i]!=0){
		Arista<T1>* aristita = new Arista<T1>(this->nodos[i], this->nodos[MatrizSolucionesDos[i]]);
...
template<typename T1>
vector<Arista<T1>*> Grafo<T1>::Busqueda_A(int id_inicio, int id_final)
{
  for(auto it: this->nodos)
  {
    vector<double> aux;
...
//  double peso_final = valores[final->data->id][1];
  ...
  auto aux = final;
  while(valores[aux->data->id][3]!=0)
  {
    recorrido.push_back(aux->data->id);
    aux = Buscar_Nodo(valores[aux->data->id][3]);
  }
  recorrido.push_back(id_inicio);
  vector<Arista<T1>*> recor;
  for(int i = 0; i < recorrido.size()-1;i++)
  {
    auto a = this->Buscar_Arista(recorrido[i+1],recorrido[i]);
    recor.push_back(a);
  }
  cout<<"Se ejecutó, la búsqueda A*, la menor distancia entre "<<id_inicio<<" y "<<id_final<<" es: "<<valores[final->data->id][1]<<endl;
  return recor;
}
